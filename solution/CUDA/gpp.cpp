...
void noflagOCC_cuWrapper(int number_bands, int ngpown, int ncouls, int *inv_igp_index, int *indinv, \
        dataType *wx_array, CustomComplex<dataType> *wtilde_array, CustomComplex<dataType> *aqsmtemp, CustomComplex<dataType> *aqsntemp,\
        CustomComplex<dataType> *I_eps_array, dataType *vcoul, dataType *achtemp_re, dataType *achtemp_im);
...
//Real and imaginary parts of achtemp calculated separately to avoid critical.
    ...
    checkCudaErrors(cudaMalloc((void**) &d_aqsmtemp, aqsmtemp_size*sizeof(CustomComplex<dataType>)));
    checkCudaErrors(cudaMalloc((void**) &d_aqsntemp, aqsntemp_size*sizeof(CustomComplex<dataType>)));

    CustomComplex<dataType> *d_I_eps_array, *d_wtilde_array;
    checkCudaErrors(cudaMalloc((void**) &d_I_eps_array, I_eps_array_size*sizeof(CustomComplex<dataType>)));
    checkCudaErrors(cudaMalloc((void**) &d_wtilde_array, wtilde_array_size*sizeof(CustomComplex<dataType>)));

    dataType *d_vcoul, *d_achtemp_re, *d_achtemp_im, *d_wx_array;
    checkCudaErrors(cudaMalloc((void**) &d_vcoul, vcoul_size*sizeof(dataType)));
    checkCudaErrors(cudaMalloc((void**) &d_wx_array, wx_array_size*sizeof(dataType)));
    checkCudaErrors(cudaMalloc((void**) &d_achtemp_re, achtemp_re_size*sizeof(dataType)));
    checkCudaErrors(cudaMalloc((void**) &d_achtemp_im, achtemp_im_size*sizeof(dataType)));

    int *d_inv_igp_index, *d_indinv;
    checkCudaErrors(cudaMalloc((void**) &d_inv_igp_index, inv_igp_index_size*sizeof(int)));
   ...
   for(int i=0; i<ncouls; i++)
    ...
    /****************************************************************************************************************/
    checkCudaErrors(cudaMemcpy(d_aqsmtemp, aqsmtemp, aqsmtemp_size*sizeof(CustomComplex<dataType>), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_aqsntemp, aqsntemp, aqsntemp_size*sizeof(CustomComplex<dataType>), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_I_eps_array, I_eps_array, I_eps_array_size*sizeof(CustomComplex<dataType>), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_wtilde_array, wtilde_array, wtilde_array_size*sizeof(CustomComplex<dataType>), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_vcoul, vcoul, vcoul_size*sizeof(dataType), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_achtemp_re, achtemp_re, achtemp_re_size*sizeof(dataType), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_achtemp_im, achtemp_im, achtemp_im_size*sizeof(dataType), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_wx_array, wx_array, wx_array_size*sizeof(dataType), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_inv_igp_index, inv_igp_index, inv_igp_index_size*sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_indinv, indinv, indinv_size*sizeof(int), cudaMemcpyHostToDevice));
    /****************************************************************************************************************/

    //The solver kernel -- this calls our MAIN LOOP (where you should focus your optimizations!)
//    noflagOCC_solver(number_bands, ngpown, ncouls, inv_igp_index, indinv, wx_array, wtilde_array, aqsmtemp, aqsntemp, I_eps_array, vcoul, achtemp_re, achtemp_im, elapsedKernelTimer);
    ...
    noflagOCC_cuWrapper(number_bands, ngpown, ncouls, d_inv_igp_index, d_indinv, d_wx_array, d_wtilde_array, d_aqsmtemp, d_aqsntemp, d_I_eps_array, d_vcoul, d_achtemp_re, d_achtemp_im);

    //COPY thre results back
    checkCudaErrors(cudaMemcpy(achtemp_re, d_achtemp_re, achtemp_re_size*sizeof(dataType), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(achtemp_im, d_achtemp_im, achtemp_re_size*sizeof(dataType), cudaMemcpyDeviceToHost));
