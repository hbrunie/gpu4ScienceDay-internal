        ...
        CustomComplex<dataType> *I_eps_array, dataType *vcoul, dataType *achtemp_re, dataType *achtemp_im)
{
    ...
    dim3 threads(32,1,1);
    printf("In noflagOCC_cuWrapper launching a cuda Kernel with grid = (%d,%d,%d), and threads = (%d,%d,%d) \n", number_bands, ngpown, 1, 32, 1, 1);

    noflagOCC_cudaSolver <<<grid,threads>>> (number_bands, ngpown, ncouls, inv_igp_index, indinv, wx_array, wtilde_array, aqsmtemp, aqsntemp, I_eps_array, vcoul, achtemp_re, achtemp_im);
