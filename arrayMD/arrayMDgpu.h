...
//Copy to Device
template<typename T>
inline void copyToDevice(T* dest, T* src, size_t size)
{
  checkCudaErrors(cudaMemcpy(dest, src, size*sizeof(T), cudaMemcpyHostToDevice));
}

//Copy to Host
template<typename T>
inline void copyToHost(T* dest, T* src, size_t size)
{
  checkCudaErrors(cudaMemcpy(dest, src, size*sizeof(T), cudaMemcpyDeviceToHost));
}
...
CUDA_HOSTDEV  inline T& operator() (size_t i1, size_t i2) { return dptr[i2+(n2*i1)]; }

CUDA_HOSTDEV   device_Array2D() { n1=n2= 0; size=0; dptr=NULL; }
CUDA_HOSTDEV   device_Array2D(const device_Array2D &p) { n1=p.n1; n2=p.n2; size=0; dptr=p.dptr; }
...
CUDA_HOSTDEV       device_Array3D(const device_Array3D &p) { n1=p.n1; n2=p.n2; n3=p.n3; size=0; dptr=p.dptr; }
